        ...
        <AppLayout user={{ first_name: '', last_name: '', middle_name: '', role: { name: 'Jail Officer' } }}>
            ...
            <div className="min-h-screen bg-slate-50">
                <div className="bg-white border-b border-slate-200 px-6 py-5 sticky top-0 z-30 shadow-sm">
                    <div className="max-w-screen-2xl mx-auto flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            <div className="p-2 bg-sky-600 rounded-xl"><BedDouble className="w-5 h-5 text-white" /></div>
                            <div>
                                <h1 className="text-lg font-bold text-slate-900 leading-none">Dormitory Management</h1>
                                <p className="text-xs text-slate-500 mt-0.5">Manage facility dormitories</p>
                            </div>
                        </div>
                        <Button onClick={openCreate} className="bg-sky-600 hover:bg-sky-700 text-white shadow-sm gap-1.5 text-sm">
                ...
                <div className="max-w-screen-2xl mx-auto px-6 py-6 space-y-6">
                    ...
                    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                        <StatCard icon={<BedDouble className="w-5 h-5" />} value={stats.total_dormitories} label="Total Dormitories" accent="bg-sky-600" iconBg="bg-sky-50" iconColor="text-sky-600" />
                    ...
                    <Tabs defaultValue="records" className="space-y-4">
                        <TabsList className="bg-white border border-slate-200 p-1 rounded-xl shadow-sm h-auto gap-1">
                            <TabsTrigger value="records" className="data-[state=active]:bg-sky-600 data-[state=active]:text-white data-[state=active]:shadow-sm rounded-lg px-4 py-2 text-sm font-medium text-slate-600 gap-2 transition-all">
                                <List className="w-4 h-4" />Dormitories
                            </TabsTrigger>
                            <TabsTrigger value="analytics" className="data-[state=active]:bg-sky-600 data-[state=active]:text-white data-[state=active]:shadow-sm rounded-lg px-4 py-2 text-sm font-medium text-slate-600 gap-2 transition-all">
                            ...
                            <Card className="border-0 shadow-sm">
                                <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className="text-xs text-slate-500 mt-0.5">{dormitories.total} dormitories total</p>
                                    </div>
                                    ...
                                    <Table>
                                        <TableHeader>
                                            <TableRow className="bg-slate-50/80 hover:bg-slate-50/80">
                                                ...
                                                <TableHead className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Building</TableHead>
                                        ...
                                        <TableBody>
                                            {dormitories.data.map((d) => (
                                                <TableRow key={d.id} className="hover:bg-slate-50 transition-colors group">
                                                    <TableCell className="pl-6"><span className="font-semibold text-slate-800 text-sm">{d.name}</span></TableCell>
                                                    <TableCell className="text-sm text-slate-600">{d.annex?.name ?? '—'}</TableCell>
                                                    ...
                                                    <TableCell className="text-right text-sm font-medium text-slate-700">{d.cells_count ?? 0}</TableCell>
                                            ...
                                            {dormitories.data.length === 0 && <TableRow><TableCell colSpan={6} className="text-center py-12 text-slate-400 text-sm">No dormitories found.</TableCell></TableRow>}
                                        </TableBody>
                                    </Table>
                                </div>
                                {dormitories.last_page > 1 && (
                                    <div className="px-6 pb-4 flex items-center justify-between pt-4 border-t border-slate-100">
                                        <p className="text-sm text-slate-500">Page {dormitories.current_page} of {dormitories.last_page} ({dormitories.total} total)</p>
                            ...
                            <div className="grid gap-4 md:grid-cols-2">
                                <Card className="border-0 shadow-sm">
                                    <div className="px-6 pt-5 pb-2 border-b border-slate-100">
                                        ...
                                        <p className="text-xs text-slate-500 mt-0.5">Distribution across dormitory types</p>
                                    </div>
                                ...
                                <Card className="border-0 shadow-sm">
                                    <div className="px-6 pt-5 pb-2 border-b border-slate-100">
                                        ...
                                        <p className="text-xs text-slate-500 mt-0.5">Capacity vs occupied beds</p>
                                    </div>
                                    <CardContent className="p-4 pt-5">
                                        <ResponsiveContainer width="100%" height={280}>
            ...
            {isCreateOpen && (
                <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
                    <DialogContent className="sm:max-w-md">
                        <DialogHeader><DialogTitle className="flex items-center gap-2"><BedDouble className="w-5 h-5 text-sky-600" />Add Dormitory</DialogTitle></DialogHeader>
                        <form onSubmit={(e) => { e.preventDefault(); createForm.post('/jail-officer/dormitories', { onSuccess: () => setIsCreateOpen(false) }); }} className="space-y-4">
                            <div><Label className="text-xs font-semibold uppercase">Building</Label>
                            ...
                            <div><Label className="text-xs font-semibold uppercase">Name</Label><Input value={createForm.data.name} onChange={(e) => createForm.setData('name', e.target.value)} required /></div>
                            <div><Label className="text-xs font-semibold uppercase">Type</Label><Input value={createForm.data.type} onChange={(e) => createForm.setData('type', e.target.value)} placeholder="e.g. Male, Female" required /></div>
                            <div><Label className="text-xs font-semibold uppercase">Description</Label><Input value={createForm.data.description} onChange={(e) => createForm.setData('description', e.target.value)} /></div>
                            <DialogFooter>
                                <Button type="button" variant="outline" onClick={() => setIsCreateOpen(false)}>Cancel</Button>
                                <Button type="submit" disabled={createForm.processing} className="bg-sky-600 hover:bg-sky-700">{createForm.processing ? 'Saving...' : 'Create Dormitory'}</Button>
                            </DialogFooter>
                        </form>
                    </DialogContent>
                </Dialog>
            ...
            {isEditOpen && selected && (
                <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
                    <DialogContent className="sm:max-w-md">
                        <DialogHeader><DialogTitle className="flex items-center gap-2"><Pencil className="w-5 h-5 text-green-600" />Edit Dormitory</DialogTitle></DialogHeader>
                        <form onSubmit={(e) => { e.preventDefault(); editForm.put(`/jail-officer/dormitories/${selected.id}`, { onSuccess: () => setIsEditOpen(false) }); }} className="space-y-4">
                            <div><Label className="text-xs font-semibold uppercase">Building</Label>
                            ...
                            <div><Label className="text-xs font-semibold uppercase">Type</Label><Input value={editForm.data.type} onChange={(e) => editForm.setData('type', e.target.value)} required /></div>
            ...
            {isDeleteOpen && selected && (
                <Dialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
                    <DialogContent className="sm:max-w-md">
                        <DialogHeader><DialogTitle className="flex items-center gap-2"><Trash2 className="w-5 h-5 text-red-600" />Delete Dormitory</DialogTitle></DialogHeader>

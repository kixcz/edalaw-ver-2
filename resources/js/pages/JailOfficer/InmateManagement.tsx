...
interface Props {
    ...
    filters: { search: string; jail_id: number | null; dormitory_id: number | null; annex_id: number | null; cell_id: number | null; status: string };
}
...
export default function InmateManagement({ inmates, cells, stats, chartData, filters }: Props) {
    ...
    const handleSearch = () => router.get('/jail-officer/inmates-hierarchical', { search: searchQuery, cell_id: cellFilter !== 'all' ? cellFilter : '', status: statusFilter !== 'all' ? statusFilter : '' }, { preserveState: true, preserveScroll: true });
        ...
        <AppLayout user={{ first_name: '', last_name: '', middle_name: '', role: { name: 'Jail Officer' } }}>
            ...
            <div className="min-h-screen bg-background">
                <div className="bg-card border-b border-border px-6 py-5 sticky top-0 z-30 shadow-sm">
                    <div className="max-w-screen-2xl mx-auto flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            <div className="p-2 bg-primary rounded-xl"><Users className="w-5 h-5 text-white" /></div>
                            <div>
                ...
                <div className="max-w-screen-2xl mx-auto px-6 py-6 space-y-6">
                    ...
                    <div className="grid w-full gap-3 sm:grid-cols-2 lg:grid-cols-4">
                        <StatCard icon={<Users className="w-5 h-5" />} value={stats.total_pdls} label="Total PDLs" accent="bg-primary" iconBg="bg-primary/10" iconColor="text-primary" />
                            ...
                            <Card className="border-0 shadow-sm">
                                <div className="px-6 py-4 border-b border-border flex items-center justify-between">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <Select value={cellFilter} onValueChange={(v) => { setCellFilter(v); router.get('/jail-officer/inmates-hierarchical', { search: searchQuery, cell_id: v !== 'all' ? v : '', status: statusFilter !== 'all' ? statusFilter : '' }, { preserveState: true, preserveScroll: true }); }}>
                                        ...
                                        <Select value={statusFilter} onValueChange={(v) => { setStatusFilter(v); router.get('/jail-officer/inmates-hierarchical', { search: searchQuery, cell_id: cellFilter !== 'all' ? cellFilter : '', status: v !== 'all' ? v : '' }, { preserveState: true, preserveScroll: true }); }}>
                                ...
                                <div className="overflow-x-auto">
                                    <Table>
                                        ...
                                        <TableBody>
                                            {inmates.data.map((i) => (
                                                <TableRow key={i.id} className="hover:bg-muted/50 transition-colors group">
                                                    <TableCell className="pl-6"><span className="font-mono text-xs bg-muted text-foreground px-2 py-1 rounded">{i.inmate_number}</span></TableCell>
                                                    <TableCell>
                                                        <div>
                                ...
                                {inmates.last_page > 1 && (
                                    <div className="px-6 pb-4 flex items-center justify-between pt-4 border-t border-border">
                                        ...
                                        <div className="flex gap-1">
                                            {inmates.current_page > 1 && <Button variant="outline" size="sm" onClick={() => router.get(`/jail-officer/inmates-hierarchical?page=${inmates.current_page - 1}`)}>Previous</Button>}
                                            {inmates.current_page < inmates.last_page && <Button variant="outline" size="sm" onClick={() => router.get(`/jail-officer/inmates-hierarchical?page=${inmates.current_page + 1}`)}>Next</Button>}
                                        </div>
                                    </div>
                                )}
                            </Card>
                                    ...
                                    <CardContent className="p-4 pt-5">
                                        <ResponsiveContainer width="100%" height={280}>
                                            <BarChart data={chartData.pdls_by_cell} margin={{ top: 5, right: 10, left: -20, bottom: 60 }}>
                                                ...
                                                <Bar dataKey="count" fill="var(--primary)" radius={[4, 4, 0, 0]} name="PDLs" />
                                            </BarChart>
                                        </ResponsiveContainer>
                                    </CardContent>
                                </Card>
            ...
            {isCreateOpen && (
                <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
                    <DialogContent className="sm:max-w-lg">
                        ...
                        <form onSubmit={(e) => { e.preventDefault(); createForm.post('/jail-officer/inmates-hierarchical', { onSuccess: () => setIsCreateOpen(false) }); }}>
            ...
            {isEditOpen && selected && (
                <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
                    <DialogContent className="sm:max-w-lg">
                        ...
                        <form onSubmit={(e) => { e.preventDefault(); editForm.put(`/jail-officer/inmates-hierarchical/${selected.id}`, { onSuccess: () => setIsEditOpen(false) }); }}>
                            <div className="space-y-4 py-4">
                                <div className="grid grid-cols-2 gap-4">
            ...
            {isTransferOpen && selected && (
                <Dialog open={isTransferOpen} onOpenChange={setIsTransferOpen}>
                    <DialogContent className="sm:max-w-lg">
                        <DialogHeader>
                            <DialogTitle>Transfer PDL</DialogTitle>
                            <DialogDescription>Move this person to a different cell assignment.</DialogDescription>
                        </DialogHeader>
                        ...
                        <form onSubmit={(e) => { e.preventDefault(); transferForm.post(`/jail-officer/inmates-hierarchical/${selected.id}/transfer`, { onSuccess: () => setIsTransferOpen(false) }); }}>
                            <div className="space-y-4 py-4">
                                <div className="space-y-2"><Label>New Cell</Label>
                            ...
                            <DialogFooter>
                                ...
                                <Button type="submit" disabled={transferForm.processing} className="bg-primary hover:bg-primary/90 text-white">{transferForm.processing ? 'Transferring...' : 'Transfer'}</Button>
                            </DialogFooter>
                        </form>
                    </DialogContent>
                </Dialog>
            ...
            {isDeleteOpen && selected && (
                <Dialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
                    <DialogContent className="sm:max-w-lg">
                        <DialogHeader>
                            <DialogTitle>Delete PDL</DialogTitle>
                            <DialogDescription>This action cannot be undone. All records for this person will be permanently removed.</DialogDescription>
                        </DialogHeader>
                        <p className="text-sm text-muted-foreground">Are you sure you want to delete <strong>{selected.first_name} {selected.last_name}</strong>?</p>
                        <DialogFooter>
                            <Button type="button" variant="outline" onClick={() => setIsDeleteOpen(false)}>Cancel</Button>
                            <Button type="button" variant="destructive" disabled={deleteForm.processing} onClick={() => deleteForm.delete(`/jail-officer/inmates-hierarchical/${selected.id}`, { onSuccess: () => setIsDeleteOpen(false) })}>

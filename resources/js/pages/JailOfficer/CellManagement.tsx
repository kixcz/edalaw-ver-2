...
type Cell = {
    id: number; cell_number: string; capacity: number; status: 'active' | 'inactive';
...
function OccupancyBar({ current, capacity }: { current: number; capacity: number }) {
    ...
    return (
        <div className="flex items-center gap-2 min-w-[120px]">
            ...
            <span className="text-xs tabular-nums text-muted-foreground shrink-0">{current}/{capacity}</span>
        </div>
    );
}
...
export default function CellManagement({ cells, stats, chartData, filters }: Props) {
    ...
    const createForm = useForm({ cell_number: '', capacity: '4', status: 'active' });
    const editForm = useForm({ cell_number: '', capacity: '4', status: 'active' });
    const deleteForm = useForm({});

    const handleSearch = () => router.get('/jail-officer/cells-hierarchical', { search: searchQuery, status: statusFilter !== 'all' ? statusFilter : '' }, { preserveState: true, preserveScroll: true });
    const openCreate = () => { setSelected(null); createForm.reset(); setIsCreateOpen(true); };
    const openEdit = (c: Cell) => { setSelected(c); editForm.setData({ cell_number: c.cell_number, capacity: String(c.capacity), status: c.status }); setIsEditOpen(true); };
                ...
                <div className="bg-card border-b border-border px-6 py-5 sticky top-0 z-30 shadow-sm">
                    <div className="max-w-screen-2xl mx-auto flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            <div className="p-2 bg-primary rounded-xl"><Grid3X3 className="w-5 h-5 text-white" /></div>
                            <div>
                                <h1 className="text-lg font-bold text-foreground leading-none">Cell Management</h1>
                                <p className="text-xs text-muted-foreground mt-0.5">Manage facility cells and occupancy</p>
                            </div>
                        </div>
                        ...
                        <TabsContent value="records">
                            <Card className="border-0 shadow-sm">
                                <div className="px-6 py-4 border-b border-border flex items-center justify-between">
                                    <div>
                                        <h3 className="font-semibold text-foreground">Cell Records</h3>
                                        <p className="text-xs text-muted-foreground mt-0.5">{cells.total} cells total</p>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        ...
                                        <Select value={statusFilter} onValueChange={(v) => { setStatusFilter(v); router.get('/jail-officer/cells-hierarchical', { search: searchQuery, status: v !== 'all' ? v : '' }, { preserveState: true, preserveScroll: true }); }}>
                                    ...
                                    <Table>
                                        <TableHeader>
                                            <TableRow className="bg-muted/50 hover:bg-muted/50">
                                                <TableHead className="text-xs font-semibold text-muted-foreground uppercase tracking-wide pl-6">Cell Number</TableHead>
                                                <TableHead className="text-xs font-semibold text-muted-foreground uppercase tracking-wide text-right">Capacity</TableHead>
                                                <TableHead className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Occupancy</TableHead>
                                        ...
                                        <TableBody>
                                            {cells.data.map((c) => (
                                                <TableRow key={c.id} className="hover:bg-muted/50 transition-colors group">
                                                    <TableCell className="pl-6"><span className="font-semibold text-foreground text-sm">{c.cell_number}</span></TableCell>
                                                    <TableCell className="text-right text-sm font-medium text-foreground">{c.capacity}</TableCell>
                                            ...
                                            {cells.data.length === 0 && <TableRow><TableCell colSpan={5} className="text-center py-12 text-muted-foreground text-sm">No cells found.</TableCell></TableRow>}
                                        </TableBody>
                                    </Table>
                                </div>
                                {cells.last_page > 1 && (
                                    <div className="px-6 pb-4 flex items-center justify-between pt-4 border-t border-border">
                                        ...
                                        <div className="flex gap-1">
                                            {cells.current_page > 1 && <Button variant="outline" size="sm" onClick={() => router.get(`/jail-officer/cells-hierarchical?page=${cells.current_page - 1}`)}>Previous</Button>}
                                            {cells.current_page < cells.last_page && <Button variant="outline" size="sm" onClick={() => router.get(`/jail-officer/cells-hierarchical?page=${cells.current_page + 1}`)}>Next</Button>}
                                        </div>
                                    </div>
                                )}
                            </Card>
                            ...
                            <div className="grid gap-4 md:grid-cols-2">
                                <Card className="border-0 shadow-sm">
                                    <div className="px-6 pt-5 pb-2 border-b border-border">
                                        <h4 className="font-semibold text-foreground text-sm">Cells by Status</h4>
                                        <p className="text-xs text-muted-foreground mt-0.5">Active vs inactive cells</p>
                                    </div>
                                ...
                                <Card className="border-0 shadow-sm">
                                    <div className="px-6 pt-5 pb-2 border-b border-border">
                                        ...
                                        <p className="text-xs text-muted-foreground mt-0.5">Capacity vs occupied beds per dormitory</p>
                                    </div>
                                    <CardContent className="p-4 pt-5">
                                        <ResponsiveContainer width="100%" height={280}>
                                            <BarChart data={chartData.occupancy_by_dormitory} margin={{ top: 5, right: 10, left: -20, bottom: 60 }}>
                                                ...
                                                <Bar dataKey="capacity" fill="var(--primary)" radius={[4, 4, 0, 0]} name="Capacity" />
                                                <Bar dataKey="occupied" fill="var(--primary)" radius={[4, 4, 0, 0]} name="Occupied" />
                                            </BarChart>
                                        </ResponsiveContainer>
                                    </CardContent>
                                </Card>
                ...
                <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
                    <DialogContent className="sm:max-w-lg">
                        <DialogHeader>
                            ...
                            <DialogDescription>Add a new cell to the facility. Fill in the required fields below.</DialogDescription>
                        </DialogHeader>
                        <form onSubmit={(e) => { e.preventDefault(); createForm.post('/jail-officer/cells-hierarchical', { onSuccess: () => setIsCreateOpen(false) }); }}>
                            <div className="space-y-4 py-4">
                                <div className="space-y-2"><Label>Cell Number</Label><Input value={createForm.data.cell_number} onChange={(e) => createForm.setData('cell_number', e.target.value)} required /></div>
                                <div className="space-y-2"><Label>Capacity</Label><Input type="number" min="1" max="50" value={createForm.data.capacity} onChange={(e) => createForm.setData('capacity', e.target.value)} required /></div>
                            </div>
                            <DialogFooter>
                                <Button type="button" variant="outline" onClick={() => setIsCreateOpen(false)}>Cancel</Button>
                                <Button type="submit" disabled={createForm.processing} className="bg-primary hover:bg-primary/90 text-white">{createForm.processing ? 'Saving...' : 'Create Cell'}</Button>
                            </DialogFooter>
                        </form>
                    </DialogContent>
                </Dialog>
                ...
                <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
                    <DialogContent className="sm:max-w-lg">
                        <DialogHeader>
                            ...
                            <DialogDescription>Update the cell information and status.</DialogDescription>
                        </DialogHeader>
                        <form onSubmit={(e) => { e.preventDefault(); editForm.put(`/jail-officer/cells-hierarchical/${selected.id}`, { onSuccess: () => setIsEditOpen(false) }); }}>
                            <div className="space-y-4 py-4">
                                <div className="space-y-2"><Label>Cell Number</Label><Input value={editForm.data.cell_number} onChange={(e) => editForm.setData('cell_number', e.target.value)} required /></div>
                                <div className="space-y-2"><Label>Capacity</Label><Input type="number" min="1" max="50" value={editForm.data.capacity} onChange={(e) => editForm.setData('capacity', e.target.value)} required /></div>
                ...
                <Dialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
                    <DialogContent className="sm:max-w-lg">
                        <DialogHeader>
                            ...
                            <DialogDescription>This action cannot be undone. The cell and its associations will be permanently removed.</DialogDescription>
                        </DialogHeader>
                        <p className="text-sm text-muted-foreground">Are you sure you want to delete <strong>{selected.cell_number}</strong>?</p>
                        <DialogFooter>
                            <Button type="button" variant="outline" onClick={() => setIsDeleteOpen(false)}>Cancel</Button>
                            <Button type="button" variant="destructive" disabled={deleteForm.processing} onClick={() => deleteForm.delete(`/jail-officer/cells-hierarchical/${selected.id}`, { onSuccess: () => setIsDeleteOpen(false) })}>
